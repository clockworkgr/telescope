...
export class ProtoStore {
  ...
  constructor(
    ...
    this.buildProto = buildProto.map((buildProtoPath) =>
      pathResolve(buildProtoPath)
    );
  ...
  processProtos(
    ...
    return contents.map(({ absolute, filename, content }) => {
      try {
        ...
        return {
          absolute,
          filename: proto.package + "/" + path.basename(filename),
          proto,
        };
      } catch (e) {
  ...
  getProtos(): ProtoRef[] {
    ...
    const allcontents = this.protoDirs.reduce((m, protoDir) => {
    ...
    const buildcontents = this.buildProto.map((filename) => ({
      absolute: filename,
      filename: filename,
      content: readFileSync(filename, "utf-8"),
    }));
